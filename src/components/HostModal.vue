...
const autoDetectPort = async () => {
  ...
  let unlisten = null;
  try {
    unlisten = await listen('lan-server-found', (event) => {
      const payload = event.payload
      if (payload && payload.port) {
        localGamePort.value = parseInt(payload.port)
        let title = payload.motd || 'Мой мир';
        if (payload.host && payload.online) {
          title = `${title} [${payload.host}, ${payload.online}]`;
        }
        roomName.value = title.substring(0, 32);
        
        if (payload.edition === 'Bedrock') {
          gameVersion.value = 'bedrock'
        } else {
          gameVersion.value = 'java'
        }
        
        portDetecting.value = false
        if (unlisten) unlisten()
      }
    })
    
    await invoke('start_lan_scanner', { edition: gameVersion.value })
    
    setTimeout(() => {
      if (portDetecting.value) {
        portDetecting.value = false
        if (unlisten) unlisten()
      }
    }, 5000)
