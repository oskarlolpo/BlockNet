...
const handleStartHost = async (config) => {
  try {
    console.log("Calling start_hosting...", config);
...
const handleConnect = async ({ password }) => {
  ...
  try {
    ...
    if (ack && ack.accepted === true) {
      ...
      const addrToUse = s.public_ip || s.peer_addrs?.[0] || '0.0.0.0:0';
      const localIp = await invoke('connect_to_peer', {
        peerId: s.client_id || s.peer_id || s.id || '',
        peerAddrs: [addrToUse],
        relaySessionId: relaySessionId,
        password: password || null
      })
      alert(`Успешно! Ваш IP для входа в Майнкрафт:\n${localIp}\n\n(IP скопирован в буфер обмена)`)
      copyEndpoint(localIp)
    } else if (ack && ack.accepted === false) {
      alert('Хост отклонил запрос (неверный пароль или занято)')
    } else {
      alert('Таймаут ожидания ответа от хоста.')
    }
        ...
        <section class="panel">
          ...
          <div class="server-list">
            ...
            <div v-else v-for="s in filteredServers" :key="s.client_id || s.id || s.peer_id || Math.random()" class="server-row">
              ...
              <template v-else>
                <button class="primary-button" @click="openConnectModal(s)" :disabled="isClient || isHosting">
                  Подключиться
                </button>
              </template>
            </div>
